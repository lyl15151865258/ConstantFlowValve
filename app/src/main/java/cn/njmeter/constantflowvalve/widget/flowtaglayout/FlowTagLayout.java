package cn.njmeter.constantflowvalve.widget.flowtaglayout;
...
public class FlowTagLayout extends ViewGroup {
    ...
    /**
     * the tag long click event callback
     */
...
//              new MarginLayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT));//这个构造方法所然能使用但是编译器会报错
                ...
                public boolean onLongClick(View view) {
                    if (mTagCheckMode == FLOW_TAG_CHECKED_NONE) {
                        if (mOnTagLongClickListener != null) {
                            mOnTagLongClickListener.onItemLongClick(FlowTagLayout.this, childView, j);
                        }
                        return true;
                    }
                    return false;
    ...
    public void setSelectedOption(int position) {
        if (position >= 0 && (mTagCheckMode == FLOW_TAG_CHECKED_SINGLE || mTagCheckMode == FLOW_TAG_CHECKED_MULTI)) {
            clearAllOption();
